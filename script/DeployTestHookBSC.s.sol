...
import {ILimitOrderManager} from "../src/lib/ILimitOrderManager.sol";

contract DeployTestHookBSC is Script {
    // BSC Pool Manager
    ...
    function run() external {
        ...
        verifyPermissions(address(hook), permissions);
    ...
    function findSalt(
        ...
        for (uint256 i = 0; i < 100000; i++) {
            ...
            // Calculate what the address would be with this salt
            bytes32 hash = keccak256(abi.encodePacked(
                ...
                keccak256(abi.encodePacked(
                    creationCode,
                    abi.encode(IPoolManager(POOL_MANAGER), LIMIT_ORDER_MANAGER, deployer)
                ))
            ));
